...
for (const file of skillFiles) {
  ...
  const effectiveContract = { ...registry.defaults, ...frontmatter };
  const missing = requiredContractFields.filter((field) => !(field in effectiveContract));
...
for (const legacy of legacyFiles) {
  const path = relative(root, legacy).replaceAll('\\', '/');
  if (!registry.legacy_files?.includes(path)) warnings.push(`legado não catalogado: ${path}`);
}
