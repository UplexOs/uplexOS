    ...
    <section id="cardapio" className="w-full bg-[var(--color-brand-cream-050)] pt-[112px] pb-[112px] overflow-hidden">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-10 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[24px]">
          {dishes.map((dish, i) => (
            <a
              href="/cardapio"
              key={i}
              className="relative group rounded-[18px] overflow-hidden aspect-[4/3] bg-[var(--color-neutral-200)] shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_24px_rgba(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-1 cursor-pointer block"
            >
              ...
              <div className="absolute bottom-4 left-4 right-4 flex flex-col justify-end">
                ...
                <span className="font-[var(--font-body)] text-[12px] font-bold text-white uppercase tracking-wider opacity-0 transform translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 mt-2 flex items-center gap-1">
                  Ver cardápio <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                </span>
              </div>
        ...
        <div className="text-center mt-[44px]">
          <a href="/cardapio" className="inline-flex items-center justify-center h-[48px] px-8 rounded-full border border-[#A62B31] text-[#A62B31] font-[var(--font-body)] font-bold text-[15px] hover:bg-[#FAF0ED] transition-colors group">
